...
if (isset($_POST['update']))
    ...
    $password_query = mysqli_query($connection,"select * from client where ID='$ID'");
    ...
    if ($database_password == $old_pass)
        ...
        if ($new_pass == $re_pass)
            {
            $update_pwd = mysqli_query($connection,"update client set password='$new_pass' where ID='$ID'");
            echo "<script>alert('Password Sucessfully Updated'); window.location='admin.php'</script>";
            }
          else
            {
            echo "<script>alert('Your new and Retype Password is not match'); window.location='cpass.php'</script>";
            }
        }
      else
        {
